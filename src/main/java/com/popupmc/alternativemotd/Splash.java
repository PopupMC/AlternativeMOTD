...
public class Splash {
    // Returns a usable splash
    // One that's <= 35 characters
    public static String randomSplash() {
        return splash.get(random.nextInt(splash.size()));
    }

    public static void removeEl(String str) {
        splash.remove(str);
    }

    // This command processes splash text, keeping only the ones of a certain size and adding them to the array
    public static void populateSplash() {
        String[] splashListRaw = {
            "PLAYER IS YOU",
            "As seen on TV!",
            "Awesome!",
            "100% pure!",
        ...
        for (String entry : splashListRaw) {
            if (entry.length() > 34)
                continue;
    ...
    public static final ArrayList<String> splash = new ArrayList<>();
}
